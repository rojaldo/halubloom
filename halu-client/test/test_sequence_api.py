# coding: utf-8

"""
    Halu API

    This should be a basic guide for the new Halu API. It will be updated as soon as I add new messages (or i'll try). The IP address to communicate with the master lamp is 10.0.0.1 by default.

    OpenAPI spec version: 1.0.0
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""


from __future__ import absolute_import

import os
import sys
import unittest

import swagger_client
from swagger_client.rest import ApiException
from swagger_client.apis.sequence_api import SequenceApi


class TestSequenceApi(unittest.TestCase):
    """ SequenceApi unit test stubs """

    def setUp(self):
        self.api = swagger_client.apis.sequence_api.SequenceApi()

    def tearDown(self):
        pass

    def test_add_sequence(self):
        """
        Test case for add_sequence

        Adds a new sequence.
        """
        pass

    def test_delete_sequence(self):
        """
        Test case for delete_sequence

        Deletes a sequence
        """
        pass

    def test_get_sequence(self):
        """
        Test case for get_sequence

        Gets information sequence from its id
        """
        pass

    def test_get_sequences(self):
        """
        Test case for get_sequences

        Gets all the sequences information
        """
        pass

    def test_modify_sequence(self):
        """
        Test case for modify_sequence

        Modifies an existing sequence.
        """
        pass


if __name__ == '__main__':
    unittest.main()
